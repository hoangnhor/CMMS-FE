...
function mapTriggerLabel(trigger) {
...
function mapAssetIcon(assetType) {
...
function canApprove(actor, item) {
  if (!actor || !item) return false;
...
function canComplete(actor, item) {
  if (!actor || !item) return false;
  if (actor.role !== "technician" || item.status !== "in_progress") return false;
  return String(item.assignedTo?._id || item.assignedTo) === String(actor._id);
}

function canSignOff(actor, item) {
  if (!actor || !item) return false;
